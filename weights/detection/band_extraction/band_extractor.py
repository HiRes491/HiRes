...
# Priority: lab_references.json (from calibrate_colors.py) → RGB fallback
# ---------------------------------------------------------------------------

_COLOR_NAME_TO_ID = {v: k for k, v in ID_TO_COLOR_NAME.items() if k > 0}

# Weights for weighted LAB distance: less L (brightness), more A/B (hue)
_W_L, _W_A, _W_B = 0.3, 1.0, 1.0

# Calibrated from training data (calibrate_colors.py):
#   black  L=1.9   brown  L=11.3   gold   L=42.9
#   silver L=64.9  white  L=61.7
_BLACK_L_THRESHOLD  =  6.0   # black(1.9) < 6 < brown(11.3)
_SILVER_WHITE_L_MIN = 55.0   # silver(64.9), white(61.7) both above 55
_SILVER_B_THRESHOLD =  2.0   # silver B≈0 (neutral), white B≈4 (warm)
_SILVER_CHROMA_MAX  =  8.0   # both silver/white have very low chroma

# Orange vs brown thresholds (from calibrated: orange A=21,B=31 vs brown A=6,B=5)
_ORANGE_A_MIN    = 12.0
_ORANGE_B_MIN    = 20.0   # red B=15 stays below; orange B=31 passes
_BROWN_A_MAX     = 10.0
_BROWN_B_MIN     = -5.0   # excludes blue (B=-31.8) and violet (B=-12.4)
_BROWN_B_MAX     = 12.0
_BROWN_CHROMA_MIN =  3.0  # excludes grey (chroma≈2.8)
_BROWN_L_MAX     = 25.0

# If nearest reference distance exceeds this, return unknown
_CONFIDENCE_THRESHOLD = 35.0


def _build_lab_references() -> dict:
    """Load calibrated references from JSON, fall back to RGB approximations."""
    import json
    json_path = os.path.join(os.path.dirname(__file__), "lab_references.json")
    if os.path.exists(json_path):
        with open(json_path) as f:
            raw = json.load(f)
        return {name: np.array(v, dtype=np.float32) for name, v in raw.items()}

    # Fallback: convert RESISTOR_RGB_REFERENCE to LAB
    refs = {}
    for name, rgb in RESISTOR_RGB_REFERENCE.items():
        patch = np.array([[list(rgb)]], dtype=np.float32) / 255.0
        bgr = patch[:, :, ::-1]
...
_LAB_REFS = _build_lab_references()


def classify_lab_color(lab: np.ndarray) -> Tuple[str, int]:
    """
    Classify a LAB color to the nearest resistor band color.

    Rules (applied in order, based on calibrated training-data LAB values):
    1. Black  — defined by very low L (not hue).
    2. Silver/White — high L, low chroma; split by B (warm vs neutral).
    3. Orange vs Brown — same hue region, separated by chroma magnitude.
    4. Weighted LAB distance to remaining reference colors.
    5. Confidence gate — reject if best distance > threshold.
    """
    L, A, B = float(lab[0]), float(lab[1]), float(lab[2])
    chroma = float(np.sqrt(A ** 2 + B ** 2))

    # Rule 1: Black — low brightness, not a hue
    if L < _BLACK_L_THRESHOLD:
        return "black", _COLOR_NAME_TO_ID["black"]

    # Rule 2: Silver / White — bright and achromatic
    if L > _SILVER_WHITE_L_MIN and chroma < _SILVER_CHROMA_MAX:
        if B < _SILVER_B_THRESHOLD:
            return "silver", _COLOR_NAME_TO_ID["silver"]
        else:
            return "white", _COLOR_NAME_TO_ID["white"]

    # Rule 3: Orange vs Brown — same hue family, very different chroma
    if A > _ORANGE_A_MIN and B > _ORANGE_B_MIN:
        return "orange", _COLOR_NAME_TO_ID["orange"]
    if (A < _BROWN_A_MAX and B > _BROWN_B_MIN and B < _BROWN_B_MAX
            and chroma > _BROWN_CHROMA_MIN and L < _BROWN_L_MAX):
        return "brown", _COLOR_NAME_TO_ID["brown"]

    # Rule 4: Weighted LAB distance
    best, best_d = "brown", float("inf")
    for name, ref in _LAB_REFS.items():
        dL = L - float(ref[0])
        dA = A - float(ref[1])
        dB = B - float(ref[2])
        d = float(np.sqrt(_W_L * dL**2 + _W_A * dA**2 + _W_B * dB**2))
        if d < best_d:
            best_d = d
            best = name

    # Rule 5: Confidence gate
    if best_d > _CONFIDENCE_THRESHOLD:
        return "unknown", 0

    return best, _COLOR_NAME_TO_ID.get(best, 3)
...
def compute_pca_axis(points: np.ndarray, debug: bool = False) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def extract_bands_by_projection(mask: np.ndarray,
                                image_rgb: np.ndarray = None,
                                n_bins: int = 200,
                                smooth_sigma: float = 2.5,
                                min_band_bins: int = 4) -> Tuple[List[BandInfo], dict]:
    """
    Extract bands by projecting every non-background pixel onto the resistor
    axis and finding contiguous color runs in the resulting 1D signal.

    Robust to fragmented segmentation: scattered pixels of the same color
    still cluster at the same axis position even when their connected
    components are broken.

    When image_rgb is provided, band color is determined by measuring the
    median LAB color of the original image pixels in each band region and
    classifying by nearest-neighbor in LAB space.  This bypasses model color
    bias entirely.

    Steps
    -----
    1. PCA axis from all non-background pixel positions.
    2. Dot-product projection → 1D coordinate t per pixel.
    3. Vote histogram: bin pixels by t, tally class votes per bin.
    4. Gaussian-smooth each class's vote curve to fill small gaps.
    5. Assign dominant class to each bin.
    6. Run-length encode → one BandInfo per contiguous same-class run.
    7. (if image_rgb) Re-classify each band via median LAB of original pixels.
    """
    ...
    axis_vector, axis_origin = compute_pca_axis(points)

    centered = points - axis_origin
    t_values = centered @ axis_vector

    class_ids = mask[ys, xs].astype(int)
    ...
    bin_idx = np.clip(
        ((t_values - t_min) / t_range * n_bins).astype(int),
        0, n_bins - 1,
    )

    # Vote histogram (n_bins × 13 classes) — used for band position detection
    vote_counts = np.zeros((n_bins, 13), dtype=np.float32)
    np.add.at(vote_counts, (bin_idx, class_ids), 1)

    smoothed = np.zeros_like(vote_counts)
    for c in range(1, 13):
        smoothed[:, c] = gaussian_filter1d(vote_counts[:, c], sigma=smooth_sigma)


    dominant = np.argmax(smoothed[:, 1:], axis=1) + 1
    no_data = vote_counts[:, 1:].sum(axis=1) == 0  # use raw votes — smoothing fills body gaps
    dominant[no_data] = 0

    # Pre-compute LAB image if needed
    image_lab = None
    if image_rgb is not None:
        img_f32 = image_rgb.astype(np.float32) / 255.0
        image_lab = cv2.cvtColor(img_f32[:, :, ::-1], cv2.COLOR_BGR2LAB)

    # Run-length encode → band candidates
    bands = []
    i = 0
    while i < n_bins:
        if dominant[i] == 0:
            i += 1
            continue
        seg_cls = int(dominant[i])
        start = i
        while i < n_bins and dominant[i] == seg_cls:
            i += 1
        end = i
        ...
        # All pixels whose t-bin falls in this run
        in_run = (bin_idx >= start) & (bin_idx < end)
        area = int(in_run.sum())
        if area < MIN_BAND_AREA:
            continue

        bx, by = xs[in_run], ys[in_run]
        bbox = (int(bx.min()), int(by.min()), int(bx.max()), int(by.max()))

        t_center = t_min + (start + end) / 2.0 * t_range / n_bins
        centroid_xy = axis_origin + t_center * axis_vector

        # ── Color classification ──────────────────────────────────────────
        if image_lab is not None:
            # Use inner 80% of band along axis to avoid edge bleed
            band_t = t_values[in_run]
            t_lo = t_min + start * t_range / n_bins
            t_hi = t_min + end   * t_range / n_bins
            margin = 0.1 * (t_hi - t_lo)
            inner = (band_t >= t_lo + margin) & (band_t <= t_hi - margin)
            sample_idx = in_run.nonzero()[0]
            if inner.sum() >= 10:
                sample_idx = sample_idx[inner]

            lab_pixels = image_lab[ys[sample_idx], xs[sample_idx]]

            # Optionally drop low-chroma pixels (near-grey — likely body bleed)
            chroma = np.sqrt(lab_pixels[:, 1] ** 2 + lab_pixels[:, 2] ** 2)
            chromatic = chroma > 8.0
            if chromatic.sum() >= max(10, 0.3 * len(lab_pixels)):
                lab_pixels = lab_pixels[chromatic]

            median_lab = np.median(lab_pixels, axis=0)
            color_name, cls = classify_lab_color(median_lab)
        else:
            cls = seg_cls
    ...
    band_projections = [
        float(np.dot(np.array(b.centroid) - axis_origin, axis_vector))
        for b in bands
    ]
    axis_info = {
        'axis_vector': axis_vector,
