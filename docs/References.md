...
| 7 | Multi-level direction heuristics | Tolerance-band localization + impossible-state detection | Priority 0 physical sanity checks (Table 3): black-edge, multi-gold/silver arrangements, strictly-interior single tolerance — each flags a segmentation artifact before direction is determined. If sane, Priority 1 uses gold/silver at an edge for direction; otherwise Priority 2 uses the tolerance-gap heuristic (Table 4) with a forward default. A post-direction check rejects invalid tolerance colors (orange/yellow/white/black at the resolved Tol slot). | Calculation |
